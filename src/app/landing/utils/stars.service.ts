...
export class StarsService {
  ...
  initGraph(): void {
    ...
    const pathArr = [
        {
          ...
          description:
            "In fact this stars' sequence is an asterism within Ursa Major. that is now the third largest of the 88 modern constellations. Also The Big Dipper is used as a symbol of north as well as flag of Alaska.",
        },
        {
          id: 1,
          path: createPathStr([
          ...
          description:
            'The Ursa Minor has been one of the most important constellation. for navigation as it contains the Polaris being as North Star',
        },
        {
          id: 2,
          path: createPathStr([
          ...
          description:
            "This constellation was named in honor of. Cassiopeia's daughter — Andromeda, in the Greek myth.",
        },
        {
          id: 3,
          path: createPathStr([
          ...
          description:
            'Scorpio is one of the brightest constellations in the sky',
        },
        {
          id: 4,
          path: createPathStr([
          ...
          description:
            "One of the Drago's star called Kepler-10 has. Kepler-10b — smallest and closest to Earth exoplanet",
        },
        {
          id: 5,
          path: createPathStr([
          ...
          description:
            'Sagittarius is one of the constellations of the zodiac. The center of the Milky Way lies in the westernmost part of Sagittarius',
        },
      ]

    // Func 0
    function createPathStr(pArr): any {
      let path = 'M'
      let index = 1
    ...
    // Func 2
    function setProps(el, classArr, attrArr): void {
      if (el !== undefined) {
        if (
          classArr !== undefined &&
          classArr.length &&
    ...
    // Func 3
    function setGroupCoords(group, rX, rY): void {
      const txy = 'translate(' + rX + ',' + rY + ')'
      group.setAttribute('transform', txy)
      svg.appendChild(group)
      return
    }
    // Func 4
    function setPath(pathStr): void {
      const pathEl: any = document.createElementNS(svgNS, 'path')
      setProps(
        pathEl,
        [],
        [
      ...
      const total_length = pathEl.getTotalElement
      pathEl.setAttribute('stroke-dasharray', total_length)
      pathEl.setAttribute('stroke-dashoffset', total_length)
      svg.appendChild(pathEl)
    }
    // Func 5
    function _setDesc(pathDesc: any): void {
      const descEl = document.createElementNS(svgNS, 'text')

      let indentX = 0
      const paragraphs = pathDesc.split('.')
      let txtNode = {}
      let tspan
      const x = (ViewportX - (67.54 / 100) * ViewportX).toFixed(0)
      const y = (ViewportY - (67.3 / 100) * ViewportY).toFixed(0)
      const txy = 'translate(' + x + ',' + y + ')'
      let verticalIndent = 20
      const textNode = paragraphs.length
      for (let i = 0; i < textNode; i++) {
    ...
    function drawStars(): void {
      ...
      // Stars Loop
      for (let i = 0, stars = 300; i < stars; i++) {
      ...
      for (let i = 0, animStars = n1; i < animStars; i++) {
        const circle: any = document.createElementNS(svgNS, 'circle')
      ...
      allCoords.forEach(function (el) {
        const elX = +el.attributes['cx'].value
        let arrElX = 0
        const elY = +el.attributes['cy'].value
        let arrElY = 0
        let pathStr = ''
        ...
        for (const item of arr) {
          arrElX = item.attributes['cx'].value
          arrElY = item.attributes['cy'].value
    ...
    // Func 1
    function drawPath(el): void {
      const element = this || el
      const id = +element.id
      const _____stars = 10
      // Return the obj according to the ID
      const currentPathObj: any = pathArr.find(function (obj) {
        return obj.id === id
      })
      const pathStr = currentPathObj.path
      const _____pathDesc = currentPathObj.description
      const _____descPos = currentPathObj.descPosition
      const pathRegex = /\w\s\d+\s\d+/gi
      const filterM = /M\s\d+\s\d+/g
      const cleanCoords = /[MLZ]\s/gi
      const xRegex = /\d+/
      const yRegex = /\d+$/
      const newCoords = []
      let X = 0
      let Y = 0
      const c = svg.querySelectorAll('.animate')
      ...
      function deleteEl(): void {
        ...
        for (let i = 0, circle = c.length; i < circle; i++) {
          if (c[i].classList.contains('paused')) {
            c[i].classList.remove('paused')
          }
        }
        if (path) {
    ...
    function doSomethingWithItem(): void {
      // Perform your action with array[index] here
      drawPath(listItems[index])

      // Move to the next item
      index = (index + 1) % (listItems.length - 1)

      // Call the function again after a delay of 2 seconds
      setTimeout(doSomethingWithItem, 1500)
    }
