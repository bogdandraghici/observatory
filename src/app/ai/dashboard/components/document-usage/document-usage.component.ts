...
@Component({
    selector: 'document-usage-widget',
    templateUrl: './document-usage.component.html',
    styleUrl: './document-usage.component.scss',
    standalone: false
})
export class DocumentUsageComponent implements OnInit, OnDestroy, OnChanges {
  @Input() hours = 1
  ...
  populateData(appId: any, hours: any): void {
    this.analyticsService.getUsageDocuments(appId, hours).then((data) => {
      ...
      this.totalRuns = calculateTotal(this.models, 'doc_cnt')
    })
  }
