...
export class QueueService {
  ...
  async createQueue(queueName: any, orgId: any): Promise<any> {
    const token = localStorage.getItem('access_token')
    const url = `${API_URL}/api/queue`
    const headers = {
  ...
  async updateQueue(updatedQueue: any): Promise<any> {
    const token = localStorage.getItem('access_token')
    const url = `${API_URL}/api/queue/${updatedQueue.id}`
    const headers = {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${token}`,
    }
