...
@Component({
    selector: 'developer-execution-list-widget',
    templateUrl: './developer-execution-list.component.html',
    styleUrls: ['./developer-execution-list.component.scss'],
    standalone: false
})
export class DeveloperExecutionListComponent implements OnInit, OnDestroy, OnChanges {
  @Input() hours = 1
  ...
  populateTable(event: any, appId: any, hours: any): void {
    ...
    this.dashboardService
      .getDeveloperExecutions(appId, hours, page, size, this.selectedAgent?.code)
      .then((data) => {
  ...
  loadAgents(): void {
    this.dashboardService
      .getDeveloperAgents(this.appId, this.hours)
      .then((data) => {
